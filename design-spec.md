...
- 2026-07-17: **Implementation complete.** All 12 AC scenarios implemented across `posts-registry.ts` (data model), `mappers.ts` (wired the API's already-present but previously-unmapped `remarks`/`studentSex` fields into `comments`/`gender`, added the `CANNOT_RESPOND` branch), `consent-forms.ts` (new `replyOnBehalf`), `RecipientFilterPopover.tsx`, `RecipientReadTable.tsx`, `ReadTrackingCards.tsx` (new `ConsentFormStatTiles`), new `EditResponseDialog.tsx`, `PostDetailPage.tsx` (orchestration, local-state patch since the mock reply endpoint doesn't mutate server-side state), MSW fixture/handler additions, and 13 E2E tests in `e2e/consent-form-responses.spec.ts` (12 scenarios, 2 tests each for the due-date-dependent ones). Full unit suite (356 tests) passes; new E2E suite (13 tests) passes; pre-existing 13-failure baseline unchanged. `pnpm lint`/`format`/`typecheck` all clean.
