...
3. **`@axe-core/playwright` was not configured** (no dependency, no global fixture). Installing it now and adding a shared `e2e/fixtures.ts` that all spec files (existing + new) import instead of importing `test`/`expect` directly from `@playwright/test`, so every E2E test — not just this feature's — gets accessibility coverage going forward.
