...
describe('postHref', () => {
  ...
  it('returns edit path for posted consent form with edit option', () => {
    const post = { kind: 'form', status: 'open', numericId: 55 } as any;
    expect(postHref(post, { edit: true })).toBe('consent-forms/55/edit');
  });
});
