    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>Choose when parents receive this post.</DialogDescription>
        </DialogHeader>

        <div className="space-y-4">
          {/* Send on — compact popover calendar field */}
          <div className="space-y-1.5">
            <Label>Send on</Label>
            <Popover open={dateOpen} onOpenChange={setDateOpen}>
              <PopoverTrigger className="inline-flex h-9 w-full items-center gap-2 rounded-[14px] border border-input bg-background px-3 text-left text-sm font-normal transition-colors hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:outline-none">
                <CalendarClock className="h-4 w-4 shrink-0 text-muted-foreground" />
                {date ? (
                  formatWhenDate(toSgtIso(date, time))
                ) : (
          ...
          {/* Send at — 15-min slots within the allowed window */}
          <div className="space-y-1.5">
            <div className="flex items-baseline justify-between gap-2">
              <Label htmlFor="schedule-time-select">Send at</Label>
